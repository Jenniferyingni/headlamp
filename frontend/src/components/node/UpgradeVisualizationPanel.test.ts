...
import {
  ...
  hasAKSManagedNodes,
  isUpgradeDetected,
} from './UpgradeVisualizationPanel';
...
function makeEvent(
  ...
  return {
    ...
    metadata: { creationTimestamp: overrides.creationTimestamp || '2025-01-01T00:00:00Z' },
  } as any;
}
...
describe('hasAKSManagedNodes', () => {
  ...
  it('detects AKS node via aks- name prefix', () => {
    const nodes = [makeNode('aks-nodepool1-12345678-vmss000000')];
    expect(hasAKSManagedNodes(nodes)).toBe(true);
  });

  it('detects AKS node via akswin name prefix', () => {
    const nodes = [makeNode('akswin000000')];
    expect(hasAKSManagedNodes(nodes)).toBe(true);
  });
...
describe('buildNodeUpgradeStates', () => {
  ...
  describe('failure paths', () => {
    ...
    it('sets failedStage for drain Warning', () => {
      ...
      const result = buildNodeUpgradeStates(nodes, events);
      expect(result.get('node-1')!.failedStage).toBe('drain');
    });
    ...
    it('sets failedStage for delete Warning', () => {
      ...
      const result = buildNodeUpgradeStates(nodes, events);
      expect(result.get('node-1')!.failedStage).toBe('deleteNode');
    });
  ...
  it('completes via Starting kubelet when node is upgrading', () => {
    ...
    const result = buildNodeUpgradeStates(nodes, events);
    expect(result.get('node-1')!.currentStage).toBe('completed');
  });

  it('completes via RegisteredNode when node is upgrading', () => {
    const nodes = [makeNode('node-1')];
    const events = [
      makeEvent({
        reason: 'Cordon',
      ...
      makeEvent({
        ...
        message: 'Registered Node node-1 in Controller',
        nodeName: 'node-1',
        creationTimestamp: '2025-01-01T00:02:00Z',
      }),
    ];
  ...
  it('does not complete via Starting kubelet when node is not upgrading', () => {
    const nodes = [makeNode('node-1')];
    const events = [
      makeEvent({
        reason: 'Starting',
    ...
    const result = buildNodeUpgradeStates(nodes, events);
    expect(result.get('node-1')!.currentStage).toBeNull();
  });
});
